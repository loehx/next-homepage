...
export const TimelineEntry: React.FC<BootstrapedTimelineEntry> = (props) => {
    ...
    const normalizedBottom = ((fromYear - yearMin) / yearsTotal) * 100;
    const top = Math.round(normalizedTop);
    const bottom = Math.floor(normalizedBottom * 100) / 100 - 1;
    const zIndex = 100 - Math.round(top);
    const odd = index % 2 !== 1;
